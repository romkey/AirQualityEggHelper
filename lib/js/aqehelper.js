var AQEHelper = {
	...
	if( options.feed ) {
	    ...
	    $( '.dashboard-updated' ).append( '<br/><a href="http://xively.com/feeds/' + id + '">raw feed</a> &bull;' );
	}

	// always add the Options link
	$( '.dashboard-updated' ).append( '<a href="chrome-extension://' + chrome.i18n.getMessage( "@@extension_id" ) + '/options.html" target="_blank">Options</a>' );
...
//	    $.get( 'https://api.cosm.com/v2/feeds/' + id + '.json', function( data ) { AQEHelper.process_raw_feed( data, options ); } );
	    ...
	    console.log( 'sending xively request' );

//		headers: { 'X-ApiKey': 'DuYsdCbKij87JJ7zoI028Kq69qZLeVdwVor5577zFd71kQhT' },
