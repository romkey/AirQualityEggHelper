...
//	    chrome.runtime.sendMessage( { subject: "test", message: "test notification" } );
	...
	if( data[ 'setup-note' ] !== undefined ) {
	    if( data[ 'setup-note' ] >= 311 )
		return;
	}

	$( 'body' ).append( '<div style="max-width: 40%; background-color: yellow; z-index: 5000; border-radius: 5px; padding: 1em; border: .5em solid black; position: fixed; top: 30%; left: 30%;"><h1>Air Quality Egg Helper Extension v3.1.1</h1><p>Corrupted sensor names are showing up in the feed used to display sensors other than temperature and humidity on this page. The extension will now only show the additional VOC, Dust and O3 sensor values by default. If you want to see all additional sensor values, go to the options and unselect the "Show only VOC, Dust and O3 extra sensors" option.</p><button onclick="$( this ).parent().hide();">Hide</button></div>' );

	chrome.storage.sync.set( { 'setup-note': 311 } );
    },
    

    add_sensor: function( symbol, options, position, value ) {
	...
	if(  options.whitelist_only && ! sensor_names.hasOwnProperty( symbol.toLowerCase() ) )
	    return;

	var long_name = sensor_names[ symbol.toLowerCase() ];
	if( ! sensor_names.hasOwnProperty( symbol.toLowerCase() ) )
	    long_name = '';
	...
	div += '  <h3 class="current-id-full">' + long_name + '</h3>\n';
	...
	for( var i in sensors ) {
	    ...
	    AQEHelper.add_sensor( symbol, options, 'left', s.current_value );
	}
